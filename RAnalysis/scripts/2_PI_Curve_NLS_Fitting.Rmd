...
ggplot(aes(x=PAR, y=Pc, color=Species, group=colony_id))+ 
  geom_point()+
  geom_line()+
  facet_wrap(~ Species*Temp.Cat.x,  ncol = 8)+
...
ests <- Data%>%
  group_by(Species, Temp.Cat.x)%>%
  summarise(Rd= mean(min(Pc)),
            Pmax = mean(max(Pc)))
...
ggplot(aes(x=Temp.Meas, y=-Rd, color=Species, group=Species))+ 
  geom_point()+
  geom_line()+
  facet_wrap(~ Species)+
  theme(legend.position = "none")
...
ggplot(aes(x=Temp.Meas, y=Pmax, color=Species, group=Species))+ 
  geom_point()+
  geom_line()+
  facet_wrap(~ Species)+
  theme(legend.position = "none")
